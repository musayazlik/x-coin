      ...
      <div className=" mt-12">
        ...
        <section className=" mb-8 sm:px-4 px-2">
          ...
          {data.freeTrainings.length > 0 ? (
            <Swippers
              data={data.freeTrainings}
              path={"/education/all-markets/commodities/"}
            />
          ) : (
            <EmptyData/>
          )}
...
export default LiveCattle;

export async function getServerSideProps(context) {
  const cookie = context.req.headers.cookie;
  const {data} = await axios.get(
    `/api/education?category=live-cattle&limit=16&page=1`,
    {
      headers: {
        cookie: cookie,
      },
