...
const ForexBreakAndIncomPage = ({data}) => {
  ...
  const blogDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`/api/dashboard/forex-break-and-incom?id=${id}`)
          .then((res) => {
            Swal.fire({
              icon: "success",
              title: "Başarılı",
        ...
        <div className="flex justify-between items-center px-4">
          <div className="flex flex-col justify-center">
            <h1 className="text-lg sm:text-2xl font-bold text-white">
              <span className="text-rose-600 border-b-2 border-rose-600">
                Forex{" "}
              </span>{" "}
              Kırılım ve Uyumsuzlukları Yönetimi
            </h1>
            <p className="sm:text-sm mt-2 text-gray-400">
              Forex Kırılım ve Uyumsuzlukları yönetmek için bu sayfayı
              kullanabilirsiniz.
            </p>
          </div>
          ...
          <button
            onClick={() => {
              router.push(`/dashboard/forex-break-and-incom/add`);
            }}
                      ...
                      data?.map((item, index) => {
                        return (
                          <tr key={item._id} className="bg-zinc-700 border-b">
                            ...
                            <td
                              className="text-sm text-zinc-400 font-light px-6 py-4 whitespace-nowrap">
                              <img
                                src={item.thumbnail}
                                alt=""
                                width={40}
                            ...
                            <td
                              className="text-sm text-zinc-400 font-medium px-6 py-4 whitespace-nowrap">
                              {item.status ? (
                                <span
                                  className="bg-green-500 text-green-700 px-2 py-1 rounded-md inline-flex justify-center border-2 border-green-800 items-center text-center">
                                    Yayında
                                  </span>
                              ) : (
                                <span
                                  className="bg-red-500 text-red-700 px-2 py-1 rounded-md inline-flex justify-center items-center text-center border-2 border-red-600">
                                    Yayında Değil
                                  </span>
                              )}
                            </td>
                            ...
                            <td
                              className="text-sm text-zinc-400 font-medium px-6 py-4 whitespace-nowrap">
                              <div
                                className="flex items-stretch justify-end gap-4 h-full ">
                                <Link
                                  href={`/dashboard/forex-break-and-incom/edit/${item._id}`}
...
export default ForexBreakAndIncomPage;

export async function getServerSideProps(context) {
  const cookie = context.req.headers.cookie;
  const {data} = await axios.get(
    `/api/dashboard/forex-break-and-incom`,
    {
      headers: {
        cookie: cookie,
      },
