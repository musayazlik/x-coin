  ...
  const blogDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`/api/dashboard/forex-alarms?id=${id}`)
          .then((res) => {
            Swal.fire({
              icon: "success",
              title: "Başarılı",
    ...
    <Layout>
      <div className="bg-zinc-800 shadow-md shadow-zinc-900/20 px-2 py-8 border-t-2 border-custom_pink">
        <div className="flex justify-between items-center px-4">
          ...
          <button
            onClick={() => {
              router.push(`/dashboard/forex-alarms/add`);
            }}
                            ...
                            <tr key={item._id} className="bg-zinc-700 border-b">
                              ...
                              <td className="text-sm text-zinc-400 font-medium px-6 py-4 whitespace-nowrap">
                                <div className="flex items-stretch justify-end gap-4 h-full ">
                                  <Link
                                    href={`/dashboard/forex-alarms/edit/${item._id}`}
...
export async function getServerSideProps(context) {
  ...
  const { data } = await axios.get(`/api/dashboard/forex-alarms`, {
    headers: {
      cookie: cookie,
    },
  });
