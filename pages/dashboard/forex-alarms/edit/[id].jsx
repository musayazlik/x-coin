      ...
      <div className="bg-zinc-800 shadow-md shadow-zinc-900/20 px-2 py-8 border-t-2 border-custom_pink">
        ...
        >
          ...
          <div className="button mt-8 flex gap-6 items-center my-4">
            ...
            <Link href="/dashboard/forex-alarms" className="text-red-700">
              İptal Et
            </Link>
          </div>
        </form>
...
export default BreakAndIncomEdit;

export async function getServerSideProps(context) {
  const cookie = context.req.headers.cookie;
  const { data } = await axios.get(
    `/api/dashboard/alarms?id=${context.params.id}`,
    {
      headers: {
        cookie: cookie,
      },
