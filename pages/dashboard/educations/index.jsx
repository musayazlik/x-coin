  ...
  const postDelete = async (e, id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios.delete(`/api/dashboard/educations?id=${id}`, {
          headers: {
            cookie: cookie,
          },
        });
      ...
      <div className="flex justify-between items-center px-4">
        <div className="flex flex-col justify-center gap-2">
          <h1 className="text-lg sm:text-2xl font-bold text-white">
            Eğitimler İçerik Yönetimi
          </h1>
          <p className="sm:text-sm text-gray-400">
            Bu sayfadan eğitimler içeriklerini yönetebilirsiniz.
          </p>
        </div>

        <button
          onClick={() => {
            router.push(`/dashboard/educations/add`);
          }}
          ...
          <div className="overflow-x-auto ">
            <div className="py-2 ">
              ...
              <span
                className="text-lg text-default-400 cursor-pointer active:opacity-50"
                onClick={() => {
                  router.push(`/dashboard/educations/edit/${item._id}`);
                }
                }
              >
...
export async function getServerSideProps(context) {
  ...
  const {data} = await axios.get(`/api/dashboard/educations`, {
    headers: {
      cookie: cookie,
    },
  });
